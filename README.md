...
| 外设类型 | 外设名称 | 引脚分配 | 功能说明 |
|---------|---------|---------|---------|
| **串口** | USART1 | PA9(TX), PA10(RX) | 调试输出，波特率115200 8-N-1 |
| **LED** | GPIO | PH10(红), PH11(绿), PH12(蓝) | RGB状态指示灯，低电平点亮 |
| **ADC** | ADC1 通道4 | PA4(模拟输入), PG3(数字量) | 光敏电阻采集，12位分辨率 |
| **以太网** | ETH (RMII模式) | PA1(REF_CLK), PA2(MDIO), PA7(CRS_DV), PB11(TX_EN), PC1(MDC), PC4(RXD0), PC5(RXD1), PG13(TXD0), PG14(TXD1), PI1(PHY_RST) | LAN8720 PHY，支持DMA收发 |
| **系统时钟** | SYSCLK | - | 180MHz主频 |
| **中断** | NVIC | - | 优先级分组4，ADC中断(优先级6)，ETH_DMA中断(优先级6) |

## 5、当你拿到本工程，你应该如何合并到主分支？
